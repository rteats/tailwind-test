    ...
    <span className={className}>
      {parts.map((part, index) => {
        if (part.startsWith('$$') && part.endsWith('$$')) {
          // Remove the $$ delimiters and render as block LaTeX
          const latex = part.slice(2, -2);
          return <BlockMath key={index} math={latex} />;
        } else if (part.startsWith('$') && part.endsWith('$')) {
          // Remove the $ delimiters and render as inline LaTeX
        ...
        <div className="max-w-2xl mx-auto text-center">
          ...
          <div className="grid gap-4 justify-center">
            <button
              ...
              className="w-64 bg-blue-600 text-white text-lg p-4 rounded-3xl cursor-pointer
              font-semibold hover:bg-blue-700 transition-colors
              shadow-xl inset-shadow-xs inset-shadow-blue-500 ring-1 ring-blue-600"
            >
              Return to Menu
            </button>
            <button
              onClick={startQuiz}
              className="w-64 bg-green-600 text-white text-lg p-4 rounded-3xl cursor-pointer
              font-semibold hover:bg-green-700 transition-colors
              shadow-xl inset-shadow-xs inset-shadow-green-500 ring-1 ring-green-600"
            >
              Restart Quiz
            </button>
          </div>
          ...
          <div className="bg-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {categories.map(category => (
                <label key={category.id} className="flex items-center p-4 border-1 border-gray-200 hover:border-gray-300 shadow-md rounded-lg hover:bg-gray-50 cursor-pointer">
                  <input
                    type="checkbox"
            ...
            {/* Start Quiz Button */}
            <div className={`max-w-2xl mx-auto px-12 p-8 flex-grow-0`}>
              <button
                onClick={startQuiz}
                disabled={selectedCategories.length === 0}
                className={`w-full cursor-pointer text-white ring-1
                  bg-blue-600
                   disabled:shadow-none disabled:inset-shadow-xs/0  disabled:ring-1 disabled:ring-gray-200 ring-blue-600 py-4
                    rounded-3xl font-semibold text-lg disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed 
                    hover:bg-blue-700 transition-all shadow-xl inset-shadow-xs inset-shadow-blue-500`}
              >
    ...
    >
      ...
      >
        ...
        <div className="bg-white mb-8">
          {/* Progress Bar */}
          <div className="bg-gray-200 rounded-full mb-8">
            <div
          ...
          {/* Display the current question's category */}
          <div className="text-center ">
            <span className="inline-block text-gray-500 text-sm font-medium px-3 py-1 rounded-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {shuffledAnswers.map((answer, index) => {
              let buttonClass = "bg-gray-100 hover:bg-gray-200 text-gray-800 inset-shadow-indigo-300";
      ...
      {showNextPrompt && (
        <div className={`max-w-2xl mx-auto w-full p-8 flex-grow-0`}>
